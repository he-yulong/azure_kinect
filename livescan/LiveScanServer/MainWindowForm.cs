...
namespace KinectServer
{
    ...
    public partial class MainWindowForm : Form
    {
        ...
        // 点击启动服务器 or 关闭服务器的按钮，调用该方法
        private void btStart_Click(object sender, EventArgs e)
        {
            // 维护两种状态（开启和关闭），不同状态进行不同的逻辑
            ...
            while (!worker.CancellationPending)
            {
                ...
                {                                 
                    ...
                    if (!oSettings.bMergeScansForSave)
                    {
                        ...
                        {
                            // 如果是文本，那么就用 XYZ 格式。
                            string outputFilename = outDir + "\\" + nFrames.ToString().PadLeft(5, '0') + i.ToString() + ".xyz";
                            Utils.saveToXYZ(outputFilename, lFrameVertsAllDevices[i], lFrameRGBAllDevices[i], oSettings.bSaveAsBinaryPLY);
                        }                      
                    }
                }
                ...
                if (oSettings.bMergeScansForSave)
                {
                    ...
                    {
                        // 如果是文本，那么就用 XYZ 格式。
                        string outputFilename = outDir + "\\" + nFrames.ToString().PadLeft(5, '0') + ".xyz";
                        Utils.saveToXYZ(outputFilename, lFrameVerts, lFrameRGB, oSettings.bSaveAsBinaryPLY);
                    }
                }
            }
        }
